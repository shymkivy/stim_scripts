...
while now*86400 < state.end_pause
    ...
    if state.lick_transition>0 % if a new lick
        if ~data.reward_type(n_trial) % if not rewarded yet continue
            ...
            if now*86400 < state.end_reward
                ...
                if state.trial_lick_rate<ops.reward_lick_rate_thersh_large
                    data.reward_type(n_trial) = 3;        % large reward
                    data.time_reward(n_trial) = now*86400 - state.start_paradigm;
                    f_write_daq_out(session, [volt,0,0,1], ops.old_daq);
