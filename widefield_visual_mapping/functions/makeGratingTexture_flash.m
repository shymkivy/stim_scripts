function [Gtxtr, TDim, Masktxtr, domains, probRatios] = makeGratingTexture_flash(Mstate, Pstate, screenPTR, screenNum, loopTrial)
    ...
    for sfid = 1:length(sfdom)
        ...
        if ~P.separable %drifting
            ...
            for colorID = 1:length(colordom)                
                Gtxtr = putinTexture(Gtxtr, screenPTR, Im,colordom,sfid,1,colorID,P);                
            end
            
            if P.blankProb > 0 && sfid == 1 %just do this once if blank is set    
                Gtxtr = putinTexture(Gtxtr, screenPTR, Im*0,colordom,length(sfdom)+1,1,1,P); %these indices signify 'blank'               
            end
            ...
            for tid = 1:length(tdom)
                ...
                for colorID = 1:length(colordom)
                    Gtxtr = putinTexture(Gtxtr, screenPTR, Im2,colordom,sfid,tid,colorID,P);
                end
                
                if P.blankProb > 0 && sfid == 1 %just do this once if blank is set
                    Gtxtr = putinTexture(Gtxtr, screenPTR, Im*0,colordom,length(sfdom)+1,tid,1,P); %index 99 signifies 'blank'
                end
...
function [Gtxtr] = putinTexture(Gtxtr, screenPTR, Im,colordom,sfid,tid,colorID,P)
