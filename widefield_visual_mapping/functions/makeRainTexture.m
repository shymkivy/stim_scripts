function [GtxtrAll, OriAll, StimLoc, TDim] = makeRainTexture(Mstate, Pstate, screenPTR, screenNum)
...
for bwid = 1:length(bwdom)
    for colorid = 1:length(colordom)
        Gtxtr(bwid,colorid) = putinTexture(screenPTR, Im*bwdom(bwid),colordom(colorid),P);
    end
end
...
function Gtxtr = putinTexture(screenPTR, Im,colortype,P)
